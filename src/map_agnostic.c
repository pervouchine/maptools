...
int main(int argc, char* argv[]) {
	...
	fprintf(stderr,"Gene information is not taken into account\n");
	...
	fprintf(stderr," -in cps3, i.e. chr1/position1/strand1 tab-delimited file, strand is +/-\n");
	...
	fprintf(stderr,"Output format: cps3+cps3, i.e. chr1/position1/strand1/chr2/position2/strand2/chainid\n");
	exit(1);
    }
    ...
    while(fgets(buff,MAXBUFFLENGTH,cps_file)) {
        ...
        i = assign_code(aux);
        m = chridx[i]+chroff[i];
        sscanf(buff,"%*s %i %c" , &position[m], &c);
...
/**********************************************************************************************/
    ...
    while(fgets(buff,MAXBUFFLENGTH,chain_file)) {
	    ...
	    for(;i<chridx[x+1]&& position[i]<end1;i++) {
		    ...
		    fprintf(out_file,"%s\t%i\t%c\t%s\t%i\t%c\t%li\n",chr1, position[i], strand_i2c(strand[i]), chr2, rescrd, strand_i2c(resstr), chain_id);
	    	}
	    }
     	}
    }
