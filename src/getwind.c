...
int main(int argc, char* argv[]) {
	...
	j = record_idx[i];
	pos[i][j] = position + (c=='3' ? coord*strand : 0);
	str[i][j] = strand_c2i(strand);
	typ[i][j] = c;
	...
	while(fgets(buff,MAXBUFFLENGTH,idx_file)) {
		...
		if(typ[i][k]=='3' || typ[i][k]=='5' || all) { 
		    if(str[i][k]>0) {
		    	p = pos[i][k] - 1 - (typ[i][k]=='3' ? exonic_window : intronic_window);
		    }
		    else {
			p = pos[i][k] - (typ[i][k]=='3' ? intronic_window : exonic_window);
		    }
		    ...
		    fprintf(outfile,"%s\t%s\t%li\t%li\t%c\t%li\t%s\t%c\n",ids[i][k], chr_name, pos[i][k] - (typ[i][k]=='3' ? coord*str[i][k] : 0), l, strand_i2c(str[i][k]), seqlen, longbuff,typ[i][k]);
		}
	    }
