...
/*******************************************************************************************************/
    ...
    while(fgets(buff,MAXBUFFLENGTH,cpsfile)) {
        if(strlen(buff)<2) break;
        sscanf(buff,"%s %*i %c %i %*i" , aux, &c, &i);
        gene_idx[i]++;
	gene_chr[i] = assign_code(aux);
	gene_str[i] = strand_c2i(c);
    }
    ...
    while(fgets(buff,MAXBUFFLENGTH,cpsfile)) {
        if(strlen(buff)<2) break;
        sscanf(buff,"%*s %i %*c %i %i %c" , &x, &i, &j, &c);
...
/**********************************************************************************************/
    ...
    while(fgets(buff,MAXBUFFLENGTH,alnfile)) {
        if(strlen(buff)<2) break;
        sscanf(buff,"%*s %*i %*c %*s %*i %*c %*i %i %*c" , &i);
	site_idx[i]++;
    }
    ...
    while(fgets(buff,MAXBUFFLENGTH,alnfile)) {
        if(strlen(buff)<2) break;
        sscanf(buff,"%*s %*i %*c %s %i %c %*i %i %*c" , &aux, &pos, &c, &i);
	site_chr[site_idx[i]+site_off[i]] = assign_code(aux);
        site_pos[site_idx[i]+site_off[i]] = pos;
        site_str[site_idx[i]+site_off[i]] = strand_c2i(c);
	...
	if(k>=0 && site_score[k]>0) {
	    fprintf(outfile,"%s\t%i\t%c\t%s\t%i\t%c\t", get_chr_name(gene_chr[i]), gene_pos[j], strand_i2c(gene_str[i]), get_chr_name(site_chr[k]), site_pos[k], strand_i2c(site_str[k]));
